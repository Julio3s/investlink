...
export default function CreateProject() {
  ...
  const [form, setForm] = useState({
    ...
    business_model: '', amount_sought: '', sector: '', country: '', status: 'brouillon',
  });
  ...
  useEffect(() => {
    if (isEdit) {
      api.get(`/projects/${id}`).then(r => {
        const p = r.data;
        setForm({ title: p.title, problem_description: p.problem_description, solution: p.solution, target_market: p.target_market, business_model: p.business_model, amount_sought: p.amount_sought, sector: p.sector || '', country: p.country || '', status: p.status });
      });
    }
  }, [id]);
    ...
    <div className="page">
      <div className="container" style={{ maxWidth: 780 }}>
        <div style={{ marginBottom: 32 }}>
          ...
          <p style={{ color: 'var(--text-2)', marginTop: 8 }}>Renseignez les informations de votre projet</p>
        </div>
              ...
              <div className="grid-2">
                ...
                <div className="form-group">
                  ...
                  <select className="input" value={form.country} onChange={e => set('country', e.target.value)}>
                    <option value="">Sélectionner</option>
                    {['Bénin','Sénégal','Côte d\'Ivoire','Mali','Burkina Faso','Togo','Niger','Cameroun','Ghana','Nigeria','France','Belgique','Canada','Autre'].map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid-2">
            ...
            <div className="grid-2">
              <div className="form-group">
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', background: 'var(--bg-3)', border: '1px dashed var(--border-light)', borderRadius: 'var(--radius)', cursor: 'pointer', fontSize: 14, color: 'var(--text-2)' }}>
                  ...
                  <input type="file" accept=".pdf" style={{ display: 'none' }} onChange={e => setFiles(f => ({ ...f, pitch_deck: e.target.files[0] }))} />
                </label>
              </div>
              <div className="form-group">
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '11px 14px', background: 'var(--bg-3)', border: '1px dashed var(--border-light)', borderRadius: 'var(--radius)', cursor: 'pointer', fontSize: 14, color: 'var(--text-2)' }}>
                  ...
                  <input type="file" accept="image/*" style={{ display: 'none' }} onChange={e => setFiles(f => ({ ...f, project_image: e.target.files[0] }))} />
                </label>
              </div>
            </div>
          </div>
