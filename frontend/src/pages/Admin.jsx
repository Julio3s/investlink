...
import { Users, Briefcase, Shield, AlertTriangle, CheckCircle, XCircle, Eye, UserX, UserCheck, BarChart2 } from 'lucide-react';
              ...
              <div>
                <div className="responsive-stats-5" style={{ marginBottom: 32 }}>
                  {[
                    ...
                    { label: 'KYC en attente', value: stats.pending_kyc, icon: Shield, color: '#f59e0b' },
                    { label: 'Signalements', value: stats.open_reports, icon: AlertTriangle, color: '#ef4444' },
                    { label: 'Conversations', value: stats.total_conversations, icon: BarChart2, color: '#10b981' },
                  ].map(({ label, value, icon: Icon, color }) => (
                    <div key={label} className="card" style={{ textAlign: 'center' }}>
                      <div style={{ width: 40, height: 40, borderRadius: 10, background: `${color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', color }}>
                        <Icon size={18} />
                      </div>
                ...
                <div style={{ padding: '20px', background: 'var(--bg-2)', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)' }}>
                  <p style={{ color: 'var(--text-2)', fontSize: 14 }}>
                    Utilisez les onglets ci-dessus pour gérer les utilisateurs, valider les KYC et traiter les signalements.
                  </p>
                </div>
              </div>
            )}
                      ...
                      {users.map(u => (
                        <tr key={u.id} style={{ borderBottom: '1px solid var(--border)' }}
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            {u.verification_status === 'verifie' ? <span className="badge badge-verified"><CheckCircle size={11}/>Vérifié</span>
                              : u.verification_status === 'en_attente' ? <span className="badge badge-pending"><Eye size={11}/>En attente</span>
                              : <span className="badge badge-rejected"><XCircle size={11}/>Non vérifié</span>}
                          </td>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
                      <div>
                        <div style={{ fontWeight: 600, marginBottom: 4 }}>
                          ...
                          {r.reported_first && <> → <span style={{ color: 'var(--danger)' }}>{r.reported_first} {r.reported_last}</span></>}
                        </div>
